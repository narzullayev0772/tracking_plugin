...
class MethodChannelGeofenceForegroundService extends GeofenceForegroundServicePlatform {
  ...
  void handleTrigger({
    ...
    backgroundChannel.setMethodCallHandler((call) async {
      ...
      return backgroundTriggerHandler(
        ...
        inputData == null ? GeofenceEventType.unKnown : (jsonDecode(inputData) as int).toGeofenceEventType(),
      );
    });
  ...
  Future<bool> isForegroundServiceRunning() async {
    ...
    try {
      isServiceRunning = await foregroundChannel.invokeMethod<bool?>('isForegroundServiceRunning') ?? false;
    } catch (e) {
      log(
        e.toString(),
  ...
  Future<bool> removeAllGeoFences() async {
    ...
    try {
      areAllAreasRemoved = await foregroundChannel.invokeMethod<bool?>('removeAllGeoFences') ?? false;
    } catch (e) {
      log(
        e.toString(),
  ...
  Future<void> subscribeToLocationUpdates() async {
    foregroundChannel.setMethodCallHandler((call) async {
      log("Location updates: ${call.arguments} ${call.method}");
    });
  }
}
